...
typedef enum { HMEMunknown=-1, HMEMtemp=0, HMEMlocal, HMEMglobal } HMEMtype;
...
extern HLibExport
Herror HXTestPtrRange(void *ptr, const char *file, INT4_8 line,
                      HBOOL do_print);

/* HXTestPtr - tests memory (address+size) allocated with HAlloc/HAllocLocal */
extern HLibExport
Herror HXTestPtr(void *ptr, const char *file, INT4_8 line);
...
extern HLibExport
Herror HSetTraceMemory(Hproc_handle proc_handle, HBOOL trace_memory);

extern HLibExport
HBOOL HGetTraceMemory(void);

/* HTmpStackSize - return stack size for temporary memory blocks             */
extern HLibExport
Herror HTmpStackSize(Hproc_handle proc_handle, INT4_8 *size);

/* HSetAllocLevel - set 'level' of memory allocation                         */
extern HLibExport
Herror HSetAllocLevel(INT4_8 level);

/* HGetAllocLevel - read 'level' of memory allocation                        */
extern HLibExport
Herror HGetAllocLevel(INT4_8 *level);

/* HGetImageCacheSize - current size in bytes of all cached images           */
extern HLibExport
Herror HGetImageCacheSize(Hproc_handle proc_handle, INT4_8 *size);

/* HGetImageCacheCapacity - maximum size in bytes of all cached images       */
extern HLibExport
Herror HGetImageCacheCapacity(Hproc_handle proc_handle, INT4_8 *size);

/* HGetImageCacheCapacity - set maximum size in bytes of all cached images
 *                      0 switches off image caching                         */
extern HLibExport
Herror HSetImageCacheCapacity(Hproc_handle proc_handle, INT4_8 size);

/* HClearImageCache - free all cached memory blocks                          */
extern HLibExport
Herror HClearImageCache(Hproc_handle proc_handle);

/* HMallocInfo - return the number of calls of clib malloc                   */
extern HLibExport
Herror HMallocInfo(INT4_8 *mcalls);

/* HMemInfo - retrieve informationen zaboutc current memory allocation       */
extern HLibExport
Herror HMemInfo(Hproc_handle proc_handle, INT4_8 *alloc_mem, INT4_8 *free_mem,
                INT4_8 *used_mem, INT4_8 *alloc_call, INT4_8 *free_call,
                double *hit_rate, double *small_rate, size_t *cache,
                size_t *stack);

/* HNumMemElements - get number of currently allocated memory blocks         */
extern HLibExport
Herror HNumMemElements(INT4_8  *num);

/* HAllocInfo - get line of call/size of all blocks with level >= AllocLevel */
extern HLibExport
Herror HAllocInfo(HINT max, char *file[], INT4 line[], INT4_8 size[],
                  INT4 level[], INT4_8 *num);
extern HLibExport
int HfprintfAllocInfo( FILE *stream);

/* HPrepMem - prepares memory (allocates+frees memory) => more efficiency    */
extern HLibExport
Herror HPrepMem (Hproc_handle proc_handle, size_t size, size_t *si_ze);

/* HFreeUnusedTmpMem - free any unused temporary memory                      */
extern HLibExport
Herror HFreeUnusedTmpMem(Hproc_handle proc_handle);
...
extern HLibExport
Herror HXAlloc (Hproc_handle proc_handle, size_t size, VOIDP void_p);

/* HXAllocAlign - allocate free usable memory with given ptr alignment */
extern HLibExport
Herror HXAllocAlign (Hproc_handle proc_handle, size_t size, size_t alignement,
                     VOIDP void_p);

/* HXAllocMemCheck - allocate free usable memory and collect test info       */
extern HLibExport
Herror HXAllocMemCheck (Hproc_handle proc_handle, size_t size,
...
extern HLibExport
Herror HXAllocAlignMemCheck (Hproc_handle proc_handle, size_t size,
                             size_t alignment, const char *file, INT4_8 line,
                             INT4 level, VOIDP void_p);

/* HXFree - give free (free usable) memory (insert it into cache) */
extern HLibExport
Herror HXFree(Hproc_handle proc_handle, VOIDP mem);

/* HXFree - give free (free usable) memory (insert it into cache) and test it*/
extern HLibExport
Herror HXFreeMemCheck(Hproc_handle proc_handle, VOIDP mem,
                      const char *file, INT4_8 line);

/* HXRealloc - reallocate memory and test it (memory still available?)       */
extern HLibExport
Herror HXRealloc(Hproc_handle proc_handle, VOIDP ptr, size_t size,
                 VOIDP void_p, const char *file, INT4_8 line);
/* HXReallocToGlobal - reallocate global/local memory and convert to global
 *  memory   */
extern HLibExport
Herror HXReallocToGlobal(Hproc_handle proc_handle, VOIDP ptr, size_t size,
                         VOIDP void_p, const char *file, INT4_8 line);

/* HXAllocRLNum - allocate permanent memory for specific region */
extern HLibExport
Herror HXAllocRLNum(Hproc_handle proc_handle,
...
extern HLibExport
Herror HXReallocRLNum(Hproc_handle proc_handle, Hrlregion *reg_old,
                      size_t num_new, Hrlregion **reg_new,
                      const char *file, INT4_8 line);

/* HXReallocRLNumToGlobal - reallocate local or global memory for specific
 * region and convert it to global memory. */
extern HLibExport
Herror HXReallocRLNumToGlobal(Hproc_handle proc_handle, Hrlregion *reg_old,
                              size_t num_new, Hrlregion **reg_new,
                              const char *file, INT4_8 line);

/* HXAllocRL - allocate permanent memory for new regions */
extern HLibExport
Herror HXAllocRL(Hproc_handle proc_handle,
                 Hrlregion *H_RESTRICT *H_RESTRICT region,
                 const char *file, INT4_8 line);

/* HXFreeRL - give free (permanent) memory of a specific region              */
extern HLibExport
Herror HXFreeRL(Hproc_handle proc_handle, Hrlregion *region,
                const char *file, INT4_8 line);


/* HXAllocGeneral - allocate memory according to allocation bit     */
extern HLibExport
Herror HXAllocGeneral(Hproc_handle ph, HINT type, size_t size, void *mem,
                      const char *file, INT4_8 line);
...
extern HLibExport
Herror HXFreeGeneral(Hproc_handle proc_handle, VOIDP mem);

extern HLibExport
Herror HXFreeGeneralMemCheck(Hproc_handle proc_handle, VOIDP mem,
                             char const*file, INT4_8 line);
...
extern HLibExport
Herror HXReallocRLNumGeneral(Hproc_handle proc_handle, Hrlregion *reg_old,
                             size_t num_new, Hrlregion **reg_new,
                             const char *file, INT4_8 line);

/* HXFreeRLGeneral - give free global/local/temp  memory of a  region        */
extern HLibExport
Herror HXFreeRLGeneral(Hproc_handle proc_handle, Hrlregion *region,
                       const char *file, INT4_8 line);

/* HAllocImageCached - fetch image memory from cache                         */
extern HLibExport
void* HAllocImageCached(size_t size);

/* HFreeImageCached - cache image memory                                     */
extern HLibExport
void HFreeImageCached(VOIDP image);
...
extern HLibExport
Herror HXAllocLocal(Hproc_handle proc_handle, size_t size, const char *file,
...
extern HLibExport
Herror HXAllocLocalGC(Hproc_handle proc_handle, size_t size, const char *file,
                      INT4_8 line, VOIDP void_p);

/* HXAllocLocalAlign - allocate local memory with given ptr alignment
                       (AllocLevel=H_LOCAL_ALLOC)           */
extern HLibExport
Herror HXAllocLocalAlign(Hproc_handle proc_handle, size_t size,
                         size_t alignment, const char *file, INT4_8 line,
                         VOIDP void_p);

/* HXFreeLocal - give free local memory (insert it in cache) and test it     */
extern HLibExport
Herror HXFreeLocal(Hproc_handle proc_handle, VOIDP mem,
                   const char *file, INT4_8 line);

/* HXReallocLocal - reallocate local memory (AllocLevel=H_LOCAL_ALLOC)       */
extern HLibExport
Herror HXReallocLocal(Hproc_handle proc_handle, VOIDP ptr, size_t size,
                      VOIDP void_p, const char *file, INT4_8 line);

/* HConvertLocalToGlobal - converts a local memory block to a global one     */
extern HLibExport
Herror HConvertLocalToGlobal(Hproc_handle proc_handle, VOIDP ptr);

/* HXAllocRLNumLocal - allocate local memory for specific region */
extern HLibExport
Herror HXAllocRLNumLocal(Hproc_handle proc_handle,
                         Hrlregion *H_RESTRICT *H_RESTRICT region, size_t num,
                         const char *file, INT4_8 line);

/* HXReallocRLNumLocal - reallocate local memory for specific region */
extern HLibExport
Herror HXReallocRLNumLocal(Hproc_handle proc_handle, Hrlregion *reg_old,
                           size_t num_new, Hrlregion **reg_new,
                           const char *file, INT4_8 line);

/* HXAllocRLLocal -  allocate local memory for new regions */
extern HLibExport
Herror HXAllocRLLocal(Hproc_handle proc_handle,
                      Hrlregion *H_RESTRICT *H_RESTRICT region,
                      const char *file, INT4_8 line);

/* HXFreeRLLocal - give free (local) memory of a specific region             */
extern HLibExport
Herror HXFreeRLLocal(Hproc_handle proc_handle, Hrlregion *region,
                     const char *file, INT4_8 line);

/* HMemClrLocal - give free the whole local memory (alloc. by HXAllocLocal)  */
extern HLibExport
Herror HMemClrLocal(Hproc_handle proc_handle);

/* HMemMoveLocal - transfers all local memory from a proc_handle to an other */
extern HLibExport
void HMemMoveLocal(Hproc_handle dest, Hproc_handle src);
